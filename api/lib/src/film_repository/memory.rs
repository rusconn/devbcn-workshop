...
pub struct MemoryFilmRepository {
    films: RwLock<HashMap<uuid::Uuid, Film>>,
}
...
impl FilmRepository for MemoryFilmRepository {
    async fn get_films(&self) -> FilmResult<Vec<Film>> {
        let result = self
            ...
            .map(|films| films.clone().into_values().collect());
    ...
    async fn create_film(&self, create_film: &CreateFilm) -> FilmResult<Film> {
        let result = self
            .films
            .write()
            .map_err(|e| format!("An error happened while trying to write a film: {:?}", e))
            .map(|mut films| {
                let film = Film {
                    id: uuid::Uuid::new_v4(),
    ...
    async fn update_film(&self, film: &Film) -> FilmResult<Film> {
        let result = self
            ...
            .map_err(|e| format!("An error happened while trying to write a film: {:?}", e))
            .and_then(|mut films| {
                if let Some(old_film) = films.get(&film.id) {
                    let mut updated_film = film.to_owned();
                    updated_film.created_at = old_film.created_at;
                    updated_film.updated_at = Some(chrono::Utc::now());
    ...
    async fn delete_film(&self, film_id: &uuid::Uuid) -> FilmResult<uuid::Uuid> {
        let result = self
            .films
            .write()
            .map_err(|e| format!("An error happened while trying to delete film: {:?}", e))
            .and_then(|mut films| {
                films
                    .remove(film_id)
                    .map(|f| f.id)
                    .ok_or_else(|| format!("Film with id {film_id} does not exist"))
            });
